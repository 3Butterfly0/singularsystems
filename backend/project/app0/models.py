...
class BuildSession(models.Model):
    ...
    # Components
    intel_cpu = models.ForeignKey(
        "intelCPU", on_delete=models.SET_NULL, null=True, blank=True
    )
    amd_cpu = models.ForeignKey(
        "amdCPU", on_delete=models.SET_NULL, null=True, blank=True
    )
    intel_motherboard = models.ForeignKey(
        "intelMotherboard", on_delete=models.SET_NULL, null=True, blank=True
    )
    amd_motherboard = models.ForeignKey(
        "amdMotherboard", on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    def estimated_watts(self):
        ...
        if cpu:
            total += cpu.wattage
        mb = (
            self.intel_motherboard if self.platform == "intel" else self.amd_motherboard
        )
        if mb:
            total += mb.wattage
...
class PrebuiltPC(models.Model):
    ...
    platform = models.CharField(max_length=50, choices=[("intel", "Intel"), ("amd", "AMD")])
